...
<template>
  <div class='float'>
    <div class='top'>
      <p>模板选择</p>
      <ul>
        <li :key="it" :class="form.template===it?'act':''" v-for="it in templates" @click="changeTem(it)" :style="{backgroundColor:it}"></li></ul>
    </div>
    <c-upload tit="自定图片" url="url"/>
    <p>
      <span>浮窗链接</span>
      <a-input v-model="form.windowUrl" placeholder="Search enter" size="large"/>
    </p>
    <a-divider orientation="left">line设置</a-divider>
    <p><span>是否启用</span>&nbsp;&nbsp;<a-switch :checked-value="1" :unchecked-value="0" v-model="form.lineStatus"/></p>
    <p>
      <span>文字</span>
    ...
    <p><span>是否启用</span>&nbsp;&nbsp;<a-switch :checked-value="1" :unchecked-value="0" v-model="form.facebookStatus"/></p>
    <p>
      <span>文字</span>
